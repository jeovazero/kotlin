...
object CirTypeRendererForUnsafeNumberAnnotation {
    fun renderType(type: CirType): String = when (type) {
    ...
    private fun renderFlexibleType(type: CirFlexibleType): String =
        "${renderType(type.lowerBound)}..${renderType(type.upperBound)}"
    ...
    private fun renderTypeArgument(typeArgument: CirTypeProjection): String {
        return when (typeArgument) {
            is CirRegularTypeProjection -> "${renderVariance(typeArgument.projectionKind)}${renderType(typeArgument.type)}"
